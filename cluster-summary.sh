...
def print_job_summary(group_list,user_list):
        ...
        for group in group_list:
                ...
                print('----------------------------------------------------------------------------------------------')
                print('{!s:32s} {:18d} {:13d} {:13d} {:14d}'.format(group[0].decode("utf-8"),group[1],group[2],group[3],group[4]))
                while ( user_list[user_cnt][0] == group[0] ):
                        user = user_list[user_cnt]
                        print('                   {!s:18s} {:13d} {:13d} {:13d} {:14d}'.format(user[1].decode("utf-8"),user[2],user[3],user[4],user[5]))
                        user_cnt += 1
                        if ( user_cnt > len(user_list) - 1 ): break

        print('----------------------------------------------------------------------------------------------')
        print('Totals:                          {:18d} {:13d} {:13d} {:14d}'.format(total_active_jobs,total_active_cores,total_pending_jobs,total_pending_cores))
        print('')
